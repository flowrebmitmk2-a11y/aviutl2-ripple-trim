...
2. 全体を詰めたいときは、タイムラインの編集メニュー、またはオブジェクトメニューから `リップル削除(重なり短縮)` を実行します。
3. 選択したフレーム範囲の中だけを詰めたいときは、あらかじめフレーム範囲選択をしてから `リップル削除(重なり短縮/選択範囲のみ)` を実行します。
4. 実行後は選択範囲が削除され、対象のオブジェクトが前へ詰まります。

## コマンドの違い

- `リップル削除(重なり短縮)`: 選択オブジェクトより後ろにあるオブジェクト全体を前へ詰めます。
- `リップル削除(重なり短縮/選択範囲のみ)`: オブジェクト選択で削除区間を決めたうえで、フレーム範囲選択の終了位置までを詰め対象にします。
